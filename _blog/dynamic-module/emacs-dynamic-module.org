...
There is not much new in the C-code, it uses the same nested function strategy as the [[http://kitchingroup.cheme.cmu.edu/blog/2017/07/09/Adding-a-GSL-integration-function-to-Emacs-with-a-dynamic-module/][integration module]]. I won't show the code here, but you can check it out [[./gsl-roots.c][here]]. The build instructions are basically the same too.

#+BEGIN_SRC c :tangle gsl-roots.c :exports none
