* Title/sentence casing bibtex entry journal titles
It seems that a best bibtex practice is to set article titles in Title case, and then let a bibtex style handle it. It seems that bibtex styles can readily translate title case to sentence case, but not the other way (http://tex.stackexchange.com/questions/10772/bibtex-loses-capitals-when-creating-bbl-file). I do not know if this is actually the rule, but it seems consistent with my experience. At least, it is try that the ACS journal style prefers title-cased article titles, but the bibtex style does not do that for you.


Here we consider how to convert all article titles to Title case. 

First, we need to decide on some rules. We will capitalize every word in a title except for these words unless they start the title: 
...
This is not a complete list. Here is some code that does most of what I want most of the time.
...
entries in articles."
  ...
  (let* ((title (bibtex-autokey-get-field "title"))
	...
	(setf (car words) (s-capitalize (car words))))
      ...
      ;;  https://github.com/jkitchin/jmax/blob/master/org/doi-utils.org#updating-bibtex-entries
      (bibtex-set-field
       "title"
...
Now, you just run this command on a bibtex entry to title case the title.
