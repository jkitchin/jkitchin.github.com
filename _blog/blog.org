...
(string-match "^fatal" (shell-command-to-string "git rev-parse --git-dir"))
#+END_SRC
...
Finally, let us get the commits with respect to a remote.

#+BEGIN_SRC emacs-lisp
(shell-command-to-string "git rev-list --count --left-right HEAD -- origin")
#+END_SRC
