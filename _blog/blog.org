* A git modeline
  ...
  :END:
I am using git more and more in Emacs, and I would like a way to know the status of the git repo I am working in by looking at the modeline. I know about magit, and other git modes, but none of them provide something as easy as useful as say git-prompt in the bash shell, which is to say I do not want to run a command to see the status.
...
Next, we would like to know how many untracked files, and modified files we have. We can get this from  =git status --porcelain=. I am going to set these to be read if they are not zero, so they stand out, and be green otherwise.
...
;; this is still a little wonky in different repos
(defun git-remote-status ()
  (interactive)
  (let* ((branch (s-trim
		  (shell-command-to-string
