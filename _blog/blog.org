...
The main difference in setting up multiple actions is that instead of a single function for action in the source definition, we provide a list of cons cells. Each action cons cell should have a descriptive string as the car that identifies the action. This will be shown in the helm buffer. The cdr should be the function to run on the candidate.

Here is an example where we have two actions. The default action will just show us the email address of the selected candidates in a message box. The second action opens an email window and inserts the selected candidates in the To: field as a comma separated list.
...
(defun some-actions (&optional candidate)
...
Here is what you do to have a fallback action from a helm session when no selections are  found. This could be an action to add an entry, accept whatever pattern you put in, etc... We just provide two sources. The second source has no candidates, and we use (dummy) in place of the candidates.
...
The variable helm-current-prefix-arg has the prefix
nil
C-u (4)
C-u C-u (16)
#+BEGIN_SRC emacs-lisp
(defun action (candidate)
  (interactive "p")
    (message-box "%s\nP = %s" candidate helm-current-prefix-arg))
