* CANCELLED Pyparsing meets Emacs to find chemical formulas
  CLOSED: [2015-06-30 Tue 20:02]


Today we expand the concepts of clickable text and merge an idea from Python with Emacs. Here we will use Python to find chemical formulas in the buffer, and then highlight them with Emacs.  We will use pyparsing to find the chemical formulas and then use them to create a pattern for button-lock. I chose this approach because regular expressions are hard to use on the most general kinds of chemical formulas, and a parser should be better equipped to handle this. I adapted an [[https://pyparsing.wikispaces.com/file/view/chemicalFormulas.py/31041705/chemicalFormulas.py][example]] grammar to match simple chemical formulas, i.e. ones that do not have any parentheses, or charges different than + or -. Here is an example usage:
...
You can see we found a few chemical formulas in the string, but we missed the H+. I don't know why.  If the string was actually our buffer, we could use those to create a regexp to put text-properties on them. The trick is how to get the buffer string to the Python function, and then get back usable information. We actually explored this [[http://kitchingroup.cheme.cmu.edu/blog/2015/05/16/Python-data-structures-to-lisp/][before]]!

The first thing we should do is work out a Python script that will output the lisp results we want, which are the found formulas (I tried getting the start and stop positions, but I don't think they map onto the buffer positions very well). Here it is. We set it up as a command line tool.
...
#+BEGIN_SRC sh
echo "Water is H2O, methane is CH4 and of course PtCl4, what about H+ and OH-? and carbon or Carbon. Water is H2O not h2o or mH2o, methane is CH4 and of course PtCl4, what about H+ and OH-? carbon, Carbon and SRC" | ./parse_chemical_formulas.py
#+END_SRC
#+RESULTS:
: '("H2O" "PtCl4" "OH-" "PtCl4" "OH-" "C")

Weird, this also misses a few, and unlike the previous example has a false hit at the end.

Now, the Emacs side to call that function. That looks like a lot of matches.

[[http://stackoverflow.com/questions/3576192/how-to-implement-shell-command-on-region-to-string-emacs-elisp-function][How to implement 'shell-command-on-region-to-string' emacs/elisp function? - Stack Overflow]]

#+BEGIN_SRC emacs-lisp
(sort '("a" "aa" "f" "fff") (lambda (a b) (> (length a) (length b))))
#+END_SRC

#+RESULTS:
| fff | aa | a | f |

We have to sort these with longest pattern first.
...
(button-lock-set-button
 (mapconcat
  'regexp-quote
  (sort  (-uniq (eval (read (shell-command-on-region-to-string (point-min) (point-max) "./parse_chemical_formulas.py"))))
...
and some tests: CH4, C2H5OH, C6H6

C6H6


Is this actually useful? Conceptually yes, I think it could be, but clearly the parsing is not recognizing formulas perfectly. Conceptually, sending the buffer to a dedicated program that can return a list of matches to highlight in Emacs is a good idea, especially if it is not easy to build in Emacs, or if a proven solution exists.
...
Benzene has a CAS number of 71-43-2. That number can be used to retrieve information about benzene, e.g.

http://www.commonchemistry.org/ChemicalDetail.aspx?ref=71-43-2

Aldrich: http://www.sigmaaldrich.com/catalog/search?term=71-43-2&interface=CAS%20No.&N=0&mode=partialmax&lang=en&region=US&focus=product
...
What? Of course that looks like an opportunity for an org-link ;)
...
Of course, you might also use button-lock for some more flexibility, say CAS: 71-43-2 or CAS 71-43-2.

cas 56-67-7

#+BEGIN_SRC emacs-lisp
(setq cas-re  "\\(CAS\\|cas\\)\\(:?\\)\\s-*\\([0-9]\\{2,7\\}-[0-9]\\{2\\}-[0-9]\\)")
(button-lock-set-button
 cas-re
 (lambda ()
   (interactive)
   (save-match-data
     (let ((s (get-surrounding-text-with-property 'cas)))
       (string-match cas-re s)
       (message-box "you clicked on cas number: %s" (match-string 3 s))
     )
...
#+RESULTS:
| \(CAS\ | cas\)\(:?\)\s-*\([0-9]\{2,7\}-[0-9]\{2\}-[0-9]\) | (0 (quote (face ((:underline t)) keymap (keymap (mouse-1 lambda nil (interactive) (save-match-data (let ((s (get-surrounding-text-with-property (quote cas)))) (string-match cas-re s) (message-box you clicked on cas number: %s (match-string 3 s)))))) button-lock t cas t mouse-face button-lock-mouse-face rear-nonsticky t)) append) |
