...
Navigation in Emacs does not have to be a House of Pain. Here we consider how to "jump" around, which means we do not use the arrows or character navigation commands to move our cursor to the place we want it.
...
- ivy-switch-buffer :: jump to a buffer
