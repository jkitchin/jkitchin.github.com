...
#+BEGIN_SRC emacs-lisp
(let* ((xml (car  (xml-parse-file "../../gilgamesh-backup/research/cu-pd-au-atat-vasp/CuPd_fcc/10/vasprun.xml")))
...
That is definitely a bit more verbose than the Python code, but I suspect that is because it uses lower level functions. In terms of actual lines of code, it is about the same because of the get_encoded_text function. The main thing missing here is the use of xpath in emacs-lisp.
