...
(setq some-helm-source
      `((name . "HELM at the Emacs")
	(candidates . ,data)
	(action . (("return" . (lambda (candidate)
                                (message-box "f1: %s" (helm-marked-candidates))))
		   ("message" . (lambda (candidate)
				  (message-box "f2: %s" candidate)))))))
