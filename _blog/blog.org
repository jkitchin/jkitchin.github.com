...
Usually the hydra menu is hard coded in the defhydra macro. Sometimes, you would like conditional options, that is, depending on some condition we get different options. Here we explore a way to achieve that. The idea is to construct the code for the hydra, then eval it, and run the hydra. In this example we make the conditional menu depend on whether we are on an even or odd second in the current time.

#+BEGIN_SRC emacs-lisp
(defun my-hydra ()
  (interactive)
  (let ((code '(defhydra my-hydra (:color blue) "My hydra")))
    ;; Conditional menu
    (add-to-list
     'code
     (if (evenp (nth 1 (current-time)))
	 '("e" (message-box "Even second") "Even")
       '("o" (message-box "Odd second") "Odd"))
     t)

     ;; create the actual hydra, then run it
     (eval code)
     (my-hydra/body)))
#+END_SRC
...
As long as it is not expensive to compute the conditionals, this seems like an easy enough way to get conditional options in a hydra.
