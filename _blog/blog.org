...
* Using the Scopus api more cleanly
We can use a namespace dictionary to make the code a little more compact. The key is using a namespace to make a "nickname" for part of the tag name.
...
# here we use the namespace to shorten the thing we find.
...
{http://www.elsevier.com/xml/svapi/abstract/dtd}coredata
 ...
 --> {http://www.elsevier.com/xml/svapi/abstract/dtd}link
      href   -> http://api.elsevier.com/content/abstract/scopus_id/77954747189
      rel   -> self
 --> {http://www.elsevier.com/xml/svapi/abstract/dtd}link
      href   -> http://www.scopus.com/inward/record.url?partnerID=HzOxMe3b&scp=77954747189&origin=inward
      rel   -> scopus
 --> {http://www.elsevier.com/xml/svapi/abstract/dtd}link
      href   -> http://api.elsevier.com/content/search/scopus?query=refeid%282-s2.0-77954747189%29
      rel   -> cited-by
...
{http://www.elsevier.com/xml/svapi/abstract/dtd}authors
 --> {http://www.elsevier.com/xml/svapi/abstract/dtd}author
      auid   -> 35787409400
      seq   -> 1
    -> {http://www.elsevier.com/xml/ani/common}initials
    -> {http://www.elsevier.com/xml/ani/common}indexed-name
    -> {http://www.elsevier.com/xml/ani/common}surname
    -> {http://www.elsevier.com/xml/ani/common}given-name
    -> {http://www.elsevier.com/xml/svapi/abstract/dtd}preferred-name
    -> {http://www.elsevier.com/xml/svapi/abstract/dtd}author-url
    -> {http://www.elsevier.com/xml/svapi/abstract/dtd}affiliation
    -> {http://www.elsevier.com/xml/svapi/abstract/dtd}affiliation
 --> {http://www.elsevier.com/xml/svapi/abstract/dtd}author
      auid   -> 7004212771
...
#print results.findall('./dtd:authors/dtd:author', ns)
print 'ffs: ', results.findall('./dtd:authors/dtd:author/dtd:affiliation', ns)

for author in results.findall('./dtd:authors/dtd:author', ns):
    #for el in author: print el.tag, el.attrib
    print (getattr(author.find('ce:indexed-name', ns), 'text', None),
           getattr(author.find('ce:given-name', ns), 'text', None),
           getattr(author.find('ce:surname', ns), 'text', None),
           getattr(author.find('dtd:author-url', ns), 'text', None),
           getattr(author.find('dtd:initials', ns), 'text', None),
           [(x.attrib['id'], x.attrib['href']) for x in author.findall('dtd:affiliation', ns)],
           author.attrib)
...
#+RESULTS:
: ffs:  [<Element '{http://www.elsevier.com/xml/svapi/abstract/dtd}affiliation' at 0x101b892d0>, <Element '{http://www.elsevier.com/xml/svapi/abstract/dtd}affiliation' at 0x101b895d0>, <Element '{http://www.elsevier.com/xml/svapi/abstract/dtd}affiliation' at 0x101b89610>]
: ('Inoglu N.', 'Nilay', u'Ino\u01e7lu', 'http://api.elsevier.com/content/author/author_id/23479647900', None, [('60027950', 'http://api.elsevier.com/content/affiliation/affiliation_id/60027950')], {'auid': '23479647900', 'seq': '1'})
: ('Kitchin J.R.', 'John R.', 'Kitchin', 'http://api.elsevier.com/content/author/author_id/7004212771', None, [('60027950', 'http://api.elsevier.com/content/affiliation/affiliation_id/60027950'), ('60026531', 'http://api.elsevier.com/content/affiliation/affiliation_id/60026531')], {'auid': '7004212771', 'seq': '2'})
...
print(getattr(coredata.find('prism:url', ns), 'text', None),
      getattr(coredata.find('dc:identifier', ns), 'text', None),
      getattr(coredata.find('dtd:eid', ns), 'text', None),
      getattr(coredata.find('prism:doi', ns), 'text', None),
      getattr(coredata.find('dc:title', ns), 'text', None),
      getattr(coredata.find('prism:aggregationType', ns), 'text', None),
      getattr(coredata.find('dtd:srctype', ns), 'text', None),
      getattr(coredata.find('dtd:citedby-count', ns), 'text', None),
      getattr(coredata.find('dc:publisher', ns), 'text', None),
      getattr(coredata.find('dtd:source-id', ns), 'text', None),
      getattr(coredata.find('prism:issn', ns), 'text', None),
      getattr(coredata.find('prism:volume', ns), 'text', None),
      getattr(coredata.find('prism:startingPage', ns), 'text', None),
      getattr(coredata.find('prism:endingPage', ns), 'text', None),
      getattr(coredata.find('prism:pageRange', ns), 'text', None),
      getattr(coredata.find('prism:coverDate', ns), 'text', None),
      getattr(coredata.find('dc:creator', ns), 'text', None),
      getattr(coredata.find('dc:description', ns), 'text', None),
)

# Get links by attribute in xpath
print coredata.find('dtd:link[@rel="scopus"]', ns).get('href', None)
print coredata.find('dtd:link[@rel="self"]', ns).get('href', None)
print coredata.find('dtd:link[@rel="cited-by"]', ns).get('href', None)
#+END_SRC
#+RESULTS:
: ('http://api.elsevier.com/content/abstract/scopus_id/84901638552', 'SCOPUS_ID:84901638552', '2-s2.0-84901638552', '10.1016/j.catcom.2013.10.028', 'Relating the electronic structure and reactivity of the 3d transition metal monoxide surfaces', 'Journal', 'j', '2', '\nElsevier\n', '22746', '15667367', '52', '60', '64', '60-64', '2014-07-05', None, None)
...
print  results.findall('dtd:affiliation', ns)


#+END_SRC

#+RESULTS:
: [<Element '{http://www.elsevier.com/xml/svapi/abstract/dtd}affiliation' at 0x101218690>]
...
class ScopusAbstract(object):
    ...
    def __init__(self, EID):
        ...
        fEID = os.path.expanduser('~/scopus-xml/get_abstract_info') + '/' + EID
        ...
        self.aggretationType = get_encoded_text(coredata, 'prism:aggregationType')
        self.publicationName = get_encoded_text(coredata, 'prism:publicationName')
        ...
        self.issueIdentifier = get_encoded_text(coredata, 'prism:issueIdentifier')
        ...
        cite_link = coredata.find('dtd:link[@rel="cited-by"]', ns).get('href', None)
        self.scopus_link = sl
    ...
    def __str__(self):
        ...
        Assumes the abstract is a journal.'''

        authors = ', '.join([a.given_name + ' ' + a.surname for a in self.authors[0:-1]])
        authors += ' and ' + self.authors[-1].given_name + ' ' + self.authors[-1].surname
...
#print ScopusAbstract('2-s2.0-84901638552')
#print ScopusAbstract('2-s2.0-84924778427')
#print ScopusAbstract('2-s2.0-77954747189')
#print ScopusAbstract('2-s2.0-77956693843')
S = ScopusAbstract('2-s2.0-77956693843')
print S

#+END_SRC
#+RESULTS:
: Nilay InoÇ§lu and John R. Kitchin, Simple model explaining and predicting coverage-dependent atomic adsorption energies on transition metal surfaces, Physical Review B - Condensed Matter and Materials Physics,82(4) Art. No. 045414, (2010-07-16). http://dx.doi.org/10.1103/PhysRevB.82.045414, http://www.scopus.com/inward/record.url?partnerID=HzOxMe3b&scp=77956693843&origin=inward, cited 14 times (Scopus).
: Affiliations:
