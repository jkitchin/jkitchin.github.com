...
So, when we work on the parent xml, we will get an outer list containing a list of a list of generators for each modeling element in the file. Sounds confusing! No worries, we will just flatten the list.
...
(defun collect-children-names (parent)
 ...
 (-flatten  (mapcar
					    (lambda (node)
					      (xml-get-children node 'generator))
					    (xml-parse-file "vasprun.xml")))
#+END_SRC
...
This returns a list containing the generator node.
#+BEGIN_SRC emacs-lisp :results code
(loop for element in (xml-node-children (car (xml-parse-file "vasprun.xml")))
      when (and (listp element)
		(eq 'generator (car element)))
      collect element)
#+END_SRC
...
To get all the i nodes we do something like this.

#+BEGIN_SRC emacs-lisp :results code
(let ((generators (loop for element in (xml-node-children (car (xml-parse-file "vasprun.xml")))
			when (and (listp element)
				  (eq 'generator (car element)))
			collect element))
      (collected '()))
  (loop for generator in generators
	do (loop for element in (xml-node-children generator)
		 when (and (listp element)
			   (eq 'i (car element)))
		 do (add-to-list 'collected element)))
  collected
  )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(find xml '((generator) (i . '((name . "program")))))
#+END_SRC
