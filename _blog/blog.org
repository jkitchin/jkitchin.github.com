...
Sometimes it is useful to convert a DOI to another type of identifier. For example, in this [[http://kitchingroup.cheme.cmu.edu/blog/2015/06/07/Getting-a-Scopus-EID-from-a-DOI/][post]] we converted a DOI to a Scopus EID. Today, we consider how to get Pubmed identifiers. Pubmed provides an API for this purpose:
...
We will call our tool "org-ref" and use the value of user-mail-address. The URL above returns XML, so we can parse it, and directly get what we want. Here is what we get.
...
Now, we just put these together to create a plist of the identifiers. For fun, we add the Scopus EID from the previous post too.
...
(let* ((url-request-method "GET")
  ...
  (list :doi doi :pmid pmid :pmcid pmcid :eid (scopus-doi-to-eid doi)))
#+END_SRC

#+RESULTS:
#+BEGIN_SRC emacs-lisp
(:doi "10.1093/nar/gks1195" :pmid "23193287" :pmcid "PMC3531190" :eid "2-s2.0-80053651587")
#+END_SRC


(require 'wos)
[[wos-search:DOI=10.1093/NAR/GKS1195]]

[[http://gateway.webofknowledge.com/gateway/Gateway.cgi?DO=(10.1093/NAR/GKS1195)&GWVersion=2&SrcApp=WEB&SrcAuth=HSB&DestApp=UA&DestLinkType=GeneralSearchSummary]]
