* A git modeline
  ...
  :END:
I am using git more and more in Emacs, and I would like a way to know the status of the git repo I am working in by looking at the modeline. I know about magit, and other git modes, but none of them provide something as easy as useful as say git-prompt in the bash shell.

Here, I want to emulate that feature in the Emacs modeline where it will show you when you are in a git repo, and then some basic information like what branch you are on, the number of untracked, modified files, and the commit status with respect to a remote. First, we only want this when we are in a git repo. We can check for that like this. The command in this block returns a string that starts with fatal when not in a git repo.
...
Let us wrap that in a nice function.
#+BEGIN_SRC emacs-lisp
(defun in-git-p ()
  (not (string-match "^fatal" (shell-command-to-string "git rev-parse --git-dir"))))
...
Next, we would like to know how many untracked files, and modified files we have. We can get this from git status:
...
Finally, let us get the commits with respect to a remote. We can do that like this. We use ^ to indicate you need to push commits up, and _ to indicate you need to pull them down.
  ...
  (apply 'format "(^%s|_%s)"
	 (split-string
	  (shell-command-to-string
	   "git rev-list --count --left-right HEAD -- origin"))))
...
Now, we can finally put this together:

#+BEGIN_SRC emacs-lisp
(setq mode-line-format '(:eval (if (not (in-git-p))
				   ""
				 (concat " git:"
					 (git-remote-status)
					 (git-parse-status)))))
...
#+RESULTS:
| :eval | (if (not (in-git-p))  (concat  git: (git-remote-status) (git-parse-status))) |
...
(add-to-list 'load-path "/Users/jkitchin/Dropbox/elisp-repos/git-emacs")
(require 'git-emacs)
