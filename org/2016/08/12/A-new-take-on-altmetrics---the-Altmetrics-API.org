...
Note: For some reason the doi and detail links in the table above seem to get scrambled in the blog post. It doesn't happen in an html export though. It seems to happen in my blog export. I guess it is time to rewrite the link handling part, since it is scrambling these!
