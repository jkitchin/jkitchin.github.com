* TODO Asynchronously running python blocks in org-mode
  :PROPERTIES:
  :categories: emacs,python,orgmode
  :date:     2015/11/20 11:46:45
  :updated:  2015/11/20 11:46:45
  :END:
...
** Emacs process approach with no tangling
   ...
   :END:
As an alternative to tangling to a file, we could get the code and send it to our process. We will get some header cruft in the first three lines of the output which I discard, and some >>> strings that look like interactive python prompts, and we get rid of that too. This seems to have a few extra blank lines in the output that I didn't find a way to eliminate yet.
